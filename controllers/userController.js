...
module.exports.changeRole = catchAsync(async (req, res, next) => {
	let updatedUser = await User.findByIdAndUpdate(req.params.id, {type: req.body.role, background: req.body.background}, {new: true, runValidators: true})
	updatedUser = updatedUser.toPublic();
	res.status(200).json({ status: "success", updatedUser, role: updatedUser.type });
});