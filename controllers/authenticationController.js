...
const adminSignupService = async (firstName, lastName, birthDate, email, username, password, type="admin") => {
...
module.exports.adminSignup = catchAsync(async (req, res, next) => {
	...
	User.validatePassword(password); //If there is an error it would be caught by catchAsync.
	 const newAdmin = await adminSignupService(firstName, lastName, birthDate, email, username, password, type)//If there is an error it would be caught by catchAsync.
...
module.exports.adminChangePassword = catchAsync(async (req, res, next) => {
	...
	const newAdmin = await User.findByIdAndUpdate(req.user._id, {
